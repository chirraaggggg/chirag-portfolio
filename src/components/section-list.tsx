        ...
        {items.map((item, index) => (
          <div key={item.title} className="group">
            <Link href={item.href} target="_blank">
              <h3 className="text-xl font-semibold mb-1 text-foreground group-hover:text-accent transition-colors duration-200">
                {item.title}
              </h3>
              <p className="text-sm text-muted-foreground mb-2">
                {item.role} {item.period && `(${item.period})`}
              </p>
              <p className="text-foreground opacity-80">{item.description}</p>
            </Link>
          </div>
        ))}
      </div>
